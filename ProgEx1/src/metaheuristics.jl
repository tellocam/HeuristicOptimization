...
#### VND ####
function vnd!(G::SPSolution, fuse_best::Bool)
    calc_time = 0
    det_const!(G, 1)
    # initial cluster size does not seem to matter.
    # we use 1 to have the bulk of the work in the local search here 

    fuse_best ? move_meths = [fuse_best!, swap_best!] : move_meths = [fuse_first!, swap_best!]
    I = 1
    totstart = time()
    while I < 3
        tstart = time()
...
function vnd_profiler!(G::SPSolution, fuse_best::Bool) #see the fraction of the time used for calc of obj_val. indicates how useful delta_eval will be
    calc_time = 0
    det_const!(G, 1)
    # initial cluster size does not seem to matter.
    # we use 1 to have the bulk of the work in the local search here 
...
#### GVNS ####
function gvns!(G::SPSolution, max_iter, shaking_meths, fuse_best::Bool)# pass list of shaking moves
    det_const!(G, G.s)
    vnd!(G, fuse_best)
